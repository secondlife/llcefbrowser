...
void multipleBrowsers::keyCallback(int key, int scancode, int action, int mods)
{
    ...
    mCefBrowserManager->SendKeyEvent(mSelectedHandle, type, glfwModsToCefModifiers(mods, key),
        glfwKeyToWindowsKeyCode(key), scancode, 0, 0, /*is_system_key=*/false);
}

void multipleBrowsers::charCallback(unsigned int codepoint)
{
    ...
    // like the URL bar must not also reach the selected browser.
    if (ImGui::GetCurrentContext() && ImGui::GetIO().WantCaptureKeyboard)
    {
        return;
    }
    ...
    mCefBrowserManager->SendKeyEvent(mSelectedHandle, llCefKeyEventType::Char, 0,
...
void multipleBrowsers::init()
{
    ...
    // See single-browser-example's own identical comment (both fields and
    // mainBundlePath/browserSubprocessPath below).
